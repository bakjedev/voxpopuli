...
uint Surface::PixelAt( int x, int y )
{
	if ( x >= 0 && x < width && y >= 0 && y < height ) {
		return pixels[y * width + x];
	} else {
		// Handle the case where the pixel coordinates are out of bounds
...
SurfaceFloat::SurfaceFloat( const char* file )
{
	LoadFromFile( file );
}

SurfaceFloat::SurfaceFloat( int w, int h, int c ) : width(w), height(h), channels(c)
{
	pixels = (float*)MALLOC64( w * h * sizeof( float ) * 3 );
	ownBuffer = true;
}

SurfaceFloat::~SurfaceFloat()
{
	if ( ownBuffer ) FREE64( pixels );
}

void SurfaceFloat::LoadFromFile( const char* file )
{
	float* data = stbi_loadf( file, &width, &height, &channels, 0 );
	if ( !data ) return; // load failed

	pixels = (float*)MALLOC64( width * height * sizeof( float ) * channels );
	ownBuffer = true; // needs to be deleted in destructor
	memcpy( pixels, data, width * height * sizeof( float ) * channels );

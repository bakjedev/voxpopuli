...
{
	bool justPressed = keystate[key & 511] == 1 && prevKeystate[key & 511] == 0;
	if ( justPressed ) {
		prevKeystate[key & 511] = 1; // Update the previous state to prevent continuous triggering
	}
	return justPressed;
}
...
void KeyEventCallback( GLFWwindow*, int key, int, int action, int )
{
	...
	if ( key >= 0 && key < 512 ) // Check if key is within array bounds
	{
		prevKeystate[key] = keystate[key]; // Store previous state
		if ( action == GLFW_PRESS ) {
			if ( app ) app->KeyDown( key );
			keystate[key] = 1;
		} else if ( action == GLFW_RELEASE ) {
			if ( app ) app->KeyUp( key );
			keystate[key] = 0;
		}
	}
}
...
void MousePosCallback( GLFWwindow*, double x, double y )
{
	...
	xscale = (float)scrwidth / (float)width;
	yscale = (float)scrheight / (float)height;
	x *= xscale, y *= yscale;
...
#ifdef FULLSCREEN
	window = glfwCreateWindow( SCRWIDTH, SCRHEIGHT, "RuhTracer", glfwGetPrimaryMonitor(), 0 );
#else
#ifdef DOUBLESIZE
	window = glfwCreateWindow( SCRWIDTH * 2, SCRHEIGHT * 2, "RuhTracer", 0, 0 );
#else
	window = glfwCreateWindow( SCRWIDTH, SCRHEIGHT, "RuhTracer", 0, 0 );
#endif
#endif
